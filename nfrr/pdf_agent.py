...
def invoke_pdf_agent(pdf_path: str):
    """
    ...
    system_prompt = SystemMessagePromptTemplate.from_template(
        template="""
                    ...
                    You will be calling a tool named `extract_md_from_pdf` with the pdf file path to convert the PDF content to markdown format.
    ...
    human_message = HumanMessagePromptTemplate.from_template(
        template="""
                        Please analyze the Markdown content from the file path :  {pdf_path}
                        ...
                        Return the output in the following format: {format} and do not include any metadata or extra information.
...
if __name__ == "__main__":
    response = invoke_pdf_agent("RBI.pdf")
    print(response)
